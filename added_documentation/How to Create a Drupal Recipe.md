...
Inside `recipes/my_recipe/composer.json` File (Optional):
   ...
   - This is noted as OPTIONAL because the `install:` Key element of your `recipe.yml` file also has the ability to [install and enable Drupal modules as outlined here.](#create-the-recipe-yml-files)   
```composer.json
{
